...
export function parseProtocolSchema(raw: string): ProtocolSchema {
  ...
  const { domains } = parseJsonInput(
    z.object({ domains: z.array(z.looseObject({ domain: z.string() })) }),
    raw,
    "the browser's /json/protocol",
  );
  return { domains };
}
...
export function protocolHelp(schema: ProtocolSchema, topic: string | undefined): string[] {
  if (topic === undefined) {
    return schema.domains.map(
      (domain) =>
        `${domain.domain}${flags(domain)}  ${domain.commands?.length ?? 0} commands, ${domain.events?.length ?? 0} events`,
    );
  }
  ...
  if (domain === undefined || extra.length > 0) {
    throw new CommandError(
      "bad_args",
      `this Chrome has no CDP domain ${domainName}`,
      "run `patchrome --profile debug cdp help` to list domains",
    );
  }
  if (memberName === undefined) {
    return [
      ...
      ...(domain.commands ?? []).map(
        (command) =>
          `  ${domain.domain}.${command.name}${flags(command)}${command.description === undefined ? "" : `  ${firstLine(command.description)}`}`,
      ),
      "",
      "events",
      ...(domain.events ?? []).map(
        (event) =>
          `  ${domain.domain}.${event.name}${flags(event)}${event.description === undefined ? "" : `  ${firstLine(event.description)}`}`,
      ),
    ];
  }
  ...
  if (member === undefined) {
    throw new CommandError(
      "bad_args",
      `${domain.domain} has no command or event ${memberName}`,
      `run \`patchrome --profile debug cdp help ${domain.domain}\``,
    );
  }
