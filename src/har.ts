...
export function buildHar(entries: NetworkEntry[], bodies: Map<string, HarBody>, creatorVersion: string) {
  ...
  return {
    log: {
      ...
      pages,
      entries: entries.filter((entry) => entry.state !== "pending").map((entry) => {
        ...
        return {
          ...
          response: {
            ...
            content: {
              size: body === undefined ? -1 : body.encoding === "base64" ? Buffer.from(body.text, "base64").byteLength : Buffer.byteLength(body.text),
...
export function harTimings(timing: RequestTiming | undefined) {
  ...
  if (!timing) return { phases: { blocked: -1, dns: -1, connect: -1, ssl: -1, send: 0, wait: 0, receive: 0 }, total: undefined };
  const phases = {
    blocked: -1,
  ...
  const total = [phases.dns, phases.connect, phases.send, phases.wait, phases.receive].filter((ms) => ms > 0).reduce((sum, ms) => sum + ms, 0);
  return { phases, total };
}
