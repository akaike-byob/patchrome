...
const context = await chromium.launchPersistentContext(mkdtempSync(join(tmpdir(), "spike4-")), {
  channel: "chrome",
  headless: false,
  viewport: null,
...
page.on("response", (response) => {
  if (response.url().includes("/blob/")) responses.push(response);
});
...
console.log(
  "page-side fetch ok/failed:",
  await page.evaluate(() => [window.ok ?? 0, window.failed ?? 0], undefined, undefined, false),
);
...
const readBodies = async (subset, label) => {
  ...
  console.log(
    `${label}: ${ok}/${subset.length} bodies ok in ${Date.now() - readStartMs} ms`,
