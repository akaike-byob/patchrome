...
async function seedEverydayChrome(fixture: FixtureServer): Promise<string> {
  ...
  const chrome = await chromium.launchPersistentContext(userDataDir, { channel: "chrome", headless: true, ignoreDefaultArgs: ["--use-mock-keychain", "--password-store=basic"] });
...
describe("state import from the everyday Chrome", () => {
  ...
  it("brings one site's cookies, localStorage and IndexedDB into the shared profile, and nothing else", async () => {
    ...
    expect(approvals.asked).toEqual([`import the 127.0.0.1 login (1 cookie and storage for 1 origin) from Chrome profile "Work" (Profile 1, ada@example.test) into patchrome profile stealth (shared by every session). Asked by agent session importer.`]);
    expect(imported.json).toMatchObject({
      ok: true,
      data: { site: "127.0.0.1", chromeProfile: { folder: "Profile 1", name: "Work" }, cookies: 1, origins: [{ origin: fixture.origin, localStorageItems: 1, indexedDB: ["auth"] }] },
    });
    ...
    expect(cookies.json.data?.cookies).toEqual([expect.objectContaining({ name: "sid", value: "from-chrome", domain: "127.0.0.1" })]);
    const token = await runCli(home, "reader", ["eval", "localStorage.getItem('token')"]);
    expect(token.json.data?.value).toBe("ls-token");
    const records = await runCli(home, "reader", ["eval", `new Promise((resolve) => {
    ...
    expect(records.json.data?.value).toEqual({ version: 3, signedInAt: "2023-11-14T22:13:20.000Z", key: [7, 8, 9], refresh: "refresh-token" });
  ...
  it("imports into an isolated session without touching the shared profile", async () => {
    ...
    const imported = await runCli(home, "sealed", ["state", "import", `${fixture.origin.replace("127.0.0.1", "localhost")}/form`, "--from", "Profile 1"], chromeEnv);
    expect(imported.json).toMatchObject({ ok: true, data: { site: "localhost", cookies: 1 } });
    expect((await runCli(home, "sealed", ["cookies", "--domain", "localhost"])).json.data?.cookies).toEqual([expect.objectContaining({ name: "sid", value: "from-chrome" })]);
    expect((await runCli(home, "reader", ["cookies", "--domain", "localhost"])).json.data?.cookies).toEqual([]);
  });
  ...
  it("copies nothing when the person does not approve, and records the refusal", async () => {
    ...
    const refused = await runCli(denyingHome, "importer", ["state", "import", "127.0.0.1", "--from", "work"], chromeEnv);
    ...
    expect(audit.json.data?.entries).toEqual([expect.objectContaining({ kind: "state-import", session: "importer", site: "127.0.0.1", decision: "denied" })]);
  });
  ...
  it("names the Chrome profiles when --from matches none, and reports a site with no login", async () => {
    ...
    expect(unknown.json.error?.hint).toBe(`pass --from with one of: "Personal" (Default), "Work" (Profile 1, ada@example.test)`);

    const empty = await runCli(home, "importer", ["state", "import", "127.0.0.1"], chromeEnv);
    expect(empty.exitCode).toBe(2);
    expect(empty.json.error?.message).toBe(`Chrome profile "Personal" (Default) has no cookies or storage for 127.0.0.1`);
  });
});
