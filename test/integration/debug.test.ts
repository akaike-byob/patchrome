...
describe("debug profile", () => {
  ...
  it("captures console messages and page errors per session", async () => {
    ...
    expect(messages.map((message) => `${message.level} ${message.text}`)).toEqual([
      "info hello 42",
      "warning careful",
      "error broken thing",
    ]);
    ...
    expect((errorsOnly.json.data?.messages as ConsoleSummary[]).map((message) => message.text)).toEqual([
      "broken thing",
    ]);
  ...
  it("streams console messages with --follow while the session keeps working", async () => {
    ...
    const lines = await new Promise<string[]>((resolve) => {
      execFile(
        process.execPath,
        [binPath, "--profile", "debug", "--session", "follow", "--timeout-ms", "1500", "console", "--follow"],
        {
  ...
  it("streams console and page errors through watch, and reads CDP help from the running Chrome", async () => {
    ...
    expect(
      lines
        ...
        .map((line) => line.replace(/^console c\d+ t\d+ /, "").replace(/ \(.*\)$/, "")),
    ).toEqual(["info hello 42", "warning careful", "error broken thing"]);
