...
describe("parseTarget", () => {
  it("takes exactly one of ref, locator or point", () => {
    ...
    expect(parseTarget({ selector: "#cb", frame: "iframe" }, { allowsPoint: true })).toEqual({ kind: "selector", selector: "#cb", frame: "iframe", nth: undefined });
    ...
    expect(() => parseTarget({ ref: "@e3", frame: "iframe" }, { allowsPoint: true })).toThrow("--frame goes with --role, --text, --label or --selector");
  ...
  it("reads role, text and label locators with --name, --exact, --nth and --frame", () => {
    expect(parseTarget({ role: "button", name: "Sign in", exact: true, nth: "1" }, { allowsPoint: false })).toEqual({ kind: "role", role: "button", name: "Sign in", isExact: true, frame: undefined, nth: 1 });
    expect(parseTarget({ role: "link" }, { allowsPoint: false })).toEqual({ kind: "role", role: "link", name: undefined, isExact: false, frame: undefined, nth: undefined });
    expect(parseTarget({ text: "More", frame: "iframe" }, { allowsPoint: false })).toEqual({ kind: "text", text: "More", isExact: false, frame: "iframe", nth: undefined });
    expect(parseTarget({ label: "Email", exact: true }, { allowsPoint: false })).toEqual({ kind: "label", label: "Email", isExact: true, frame: undefined, nth: undefined });
    ...
    expect(() => parseTarget({ role: "button", nth: "-1" }, { allowsPoint: false })).toThrow("--nth must be a whole number");
...
describe("challenge vendors", () => {
  it("names widget frames by host and path", () => {
    expect(vendorOfFrameUrl("https://challenges.cloudflare.com/cdn-cgi/challenge-platform/h/b/turnstile/if/ov2")).toBe("turnstile");
...
describe("cli targets", () => {
  ...
  it("reads fill's only positional as text when --selector is given", () => {
    expect(parse(["fill", "--selector", "#code", "--frame", "iframe", "ab1"])).toMatchObject({ command: "fill", args: { selector: "#code", frame: "iframe", fillText: "ab1" } });
  ...
  it("gives a person ten minutes on a challenge handoff", () => {
    expect(parse(["challenge", "--handoff"])).toMatchObject({ command: "challenge", timeoutMs: 600_000, args: { handoff: true } });
