...
describe("wait conditions", () => {
  it("takes exactly one condition, with --gone flipping it", () => {
    expect(parseWaitCondition({ title: "Prove your humanity", gone: true })).toEqual({
      kind: "title",
      text: "Prove your humanity",
      isGone: true,
    });
    expect(parseWaitCondition({ selector: "#posts", gone: false })).toEqual({
      kind: "element",
      locator: { kind: "selector", selector: "#posts", frame: undefined, nth: undefined },
      isGone: false,
    });
    expect(parseWaitCondition({ role: "button", name: "Next", gone: true })).toEqual({
      kind: "element",
      locator: { kind: "role", role: "button", name: "Next", isExact: false, frame: undefined, nth: undefined },
      isGone: true,
    });
    ...
    for (const args of [
      ...
      expect(
        errorCodeOf(() => parseWaitCondition(args)),
        JSON.stringify(args),
      ).toBe("bad_args");
    }
  });
...
describe("watch events", () => {
  ...
  it("prints one line per event", () => {
    expect(watchEventLine({ kind: "navigation", tabId: "t2", url: "https://a.test/", atMs: 0 })).toBe(
      "navigation t2 https://a.test/",
    );
  });
});
...
describe("cdp help", () => {
  ...
  it("lists domains, a domain's members, and one member's params", () => {
    expect(protocolHelp(schema, undefined)).toEqual([
      "Page  1 commands, 1 events",
      "WebMCP [experimental]  0 commands, 0 events",
    ]);
...
describe("parseCli for wait, watch, sessions and completions", () => {
  it("maps the new commands", () => {
    expect(parse(["wait", "--title", "Prove your humanity", "--gone"])).toMatchObject({
      command: "wait",
      timeoutMs: 30_000,
      args: { title: "Prove your humanity", gone: true },
    });
    expect(parse(["watch", "--events", "response", "--url", "*api*", "--count", "2"])).toMatchObject({
      command: "watch",
      timeoutMs: 600_000,
      args: { events: "response", url: "*api*", count: 2 },
    });
    expect(parse(["cdp", "help", "Page.navigate"])).toMatchObject({
      command: "cdp-help",
      args: { topic: "Page.navigate" },
    });
    expect(parse(["cdp", "help"])).toMatchObject({ command: "cdp-help", args: { topic: undefined } });
    expect(parse(["sessions", "work-*"])).toMatchObject({
      command: "sessions",
      args: { pattern: "work-*" },
      shouldStartDaemon: false,
    });
    expect(parse(["session", "close", "work-*"])).toMatchObject({
