...
describe("locatorForRef", () => {
  it("turns a unique role and name into an exact locator", () => {
    expect(locatorForRef(snapshot, "e3")).toEqual({
      flags: ["--role", "textbox", "--name", "Email", "--exact"],
      notes: [],
    });
  ...
  it("adds --nth for repeated and nameless nodes, with a note", () => {
    expect(locatorForRef(snapshot, "e6")).toEqual({
      flags: ["--role", "button", "--name", "Add to cart", "--exact", "--nth", "1"],
      notes: [expect.stringContaining("2 elements matched")],
    });
  ...
  it("treats the root's frame prefix as the page", () => {
    const prefixed =
      '- generic [ref=f1e1]:\n  - button "Go" [ref=f1e2]\n  - iframe [ref=f1e3]:\n    - button "Go" [ref=f2e1]';
    expect(locatorForRef(prefixed, "f1e2")).toEqual({
      flags: ["--role", "button", "--name", "Go", "--exact"],
      notes: [],
    });
  ...
  it("notes refs inside iframes and returns undefined for unknown refs", () => {
    expect(locatorForRef(snapshot, "f1e2")).toEqual({
      flags: ["--role", "checkbox", "--name", "Verify", "--exact"],
      notes: [expect.stringContaining("--frame")],
    });
...
describe("replayStep", () => {
  it("drops caller flags and replaces a positional ref", () => {
    const step = replayStep(
      "click",
      ["--json", "--session", "s1", "click", "@e5"],
      { refLocator: { flags: ["--role", "button"], notes: ["n"] } },
      5,
    );
  ...
  it("replaces --ref with its value, request ids and secret text", () => {
    expect(
      replayStep(
        "text",
        ["text", "--ref", "e2", "--inline"],
        { refLocator: { flags: ["--role", "heading"], notes: [] } },
        0,
      ).argv,
    ).toEqual(["text", "--role", "heading", "--inline"]);
    expect(
      replayStep("text", ["text", "--ref=e2"], { refLocator: { flags: ["--role", "heading"], notes: [] } }, 0).argv,
    ).toEqual(["text", "--role", "heading"]);
    expect(
      replayStep("network-get", ["network", "get", "n17", "--body"], { requestUrlGlob: "https://a.test/api*" }, 0).argv,
    ).toEqual(["network", "get", "--url", "https://a.test/api*", "--body"]);
    const secret = replayStep(
      "fill",
      ["fill", "@e3", "hunter2"],
      { refLocator: { flags: ["--label", "Password"], notes: [] }, isSecretText: true },
      0,
    );
  ...
  it("keeps words it has no hint for, and notes tab ids", () => {
    expect(stepArgv(["--profile", "debug", "--timeout-ms", "900", "goto", "https://a.test"])).toEqual([
      "--timeout-ms",
      "900",
      "goto",
      "https://a.test",
    ]);
    expect(replayStep("switch", ["switch", "t3"], undefined, 0).notes).toEqual([
      "tab ids from the recording can differ on replay",
    ]);
  });
});
